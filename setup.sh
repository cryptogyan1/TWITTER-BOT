...
echo "📁 Creating project folder and cloning repository..."
mkdir -p twitter-tracker-bot && cd twitter-tracker-bot
git clone https://github.com/0xStarLabs/TwitterAPI.git
cd TwitterAPI
...
# Ask user if they want to edit .env
read -p "🛠️  Do you want to edit the .env file now? (y/n): " edit_env
if [[ "$edit_env" == "y" || "$edit_env" == "Y" ]]; then
  nano .env
fi

# Ask user if they want to run the bot
read -p "🚀 Do you want to run the bot now? (y/n): " run_bot
if [[ "$run_bot" == "y" || "$run_bot" == "Y" ]]; then
  echo "▶️ Starting bot..."
  node bot.js
else
  echo "✅ Setup complete. You can run the bot anytime with: node bot.js"
fi
